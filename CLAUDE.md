...
`scripts/parse_pdf.py` deterministically parses `Pruvodce_Socialnimi_Sluzbami.pdf` into `strapi/services.json`. Re-run it whenever the source PDF changes.
...
The script validates output (broken refs, duplicate IDs, missing contacts) and prints warnings to stderr.

## Key Files

- `compose.yaml` - Docker services configuration
- `strapi/services.json` - Source data from PDF (generated by `scripts/parse_pdf.py`)
- `scripts/parse_pdf.py` - PDF parser (Python, uses pdfplumber)
