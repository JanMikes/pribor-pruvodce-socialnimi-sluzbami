      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-8">
          {authorities.map((authority) => (
            ...
            >
              {/* Authority Header */}
              <div className="bg-accent-50 px-6 py-5 border-b border-accent-100">
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 bg-accent-600 text-white rounded-xl flex items-center justify-center flex-shrink-0">
                    ...
                    <div className="space-y-1 text-sm text-gray-600">
                      ...
                      {authority.website && (
                        <div className="flex items-center gap-2">
                          ...
                          <a href={authority.website} target="_blank" rel="noopener noreferrer" className="hover:text-primary-600">
                            {authority.website}
                          </a>
                        </div>
                      )}
                    ...
                    <div key={department.id} className="px-6 py-5">
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        {department.phone && (
                          <div className="flex items-center gap-2 text-gray-600">
                            ...
                            <a href={`tel:${department.phone}`} className="hover:text-primary-600">
                              {department.phone}
                            </a>
                          </div>
                        )}
                        {department.email && (
                          <div className="flex items-center gap-2 text-gray-600">
                            ...
                            <a href={`mailto:${department.email}`} className="hover:text-primary-600">
                              {department.email}
                            </a>
                          </div>
                        )}
                      ...
                      {department.contacts && department.contacts.length > 0 && (
                        <div className="mt-4 pt-4 border-t border-gray-100">
                          <h4 className="text-xs font-medium text-gray-500 uppercase mb-3">Kontaktní osoby</h4>
                            ...
                            {department.contacts.map((contact, idx) => (
                              <div key={idx} className="bg-gray-50 rounded-lg p-3">
                                ...
                                <div className="space-y-1 mt-1 text-sm">
                                  {contact.phone && (
                                    <a href={`tel:${contact.phone}`} className="block text-gray-600 hover:text-primary-600">
                                      {contact.phone}
                                    </a>
                                  )}
                                  {contact.email && (
                                    <a href={`mailto:${contact.email}`} className="block text-gray-600 hover:text-primary-600 truncate">
                                      {contact.email}
                                    </a>
                                  )}
                                </div>
